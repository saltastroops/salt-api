...
class BaseBlockVisit(BaseModel):
    ...
    night: date = Field(
        ...
        description=(
            "Start date of the night when the observation was made (or the block visit"
            " was queued)."
        ),
    )
...
class BlockVisit(BaseBlockVisit):
    ...
    targets: List[str] = Field(
        ...
        title="Targets",
        description=(
            "List of the names of the observed targets. With the exception of a few"
            " legacy observations, the list contains a single target"
        ),
    )
...
class ObservationProbabilities(BaseModel):
    ...
    moon: Optional[float] = Field(
        ...
        description=(
            "Moon probability, which is derived from the lunar phase cumulative"
            " distribution function during the semester. The moon probability is not"
            " used in the total probability calculation since the moon constraints are"
            " already incorporated in the visibility window calculations"
        ),
    )
    competition: Optional[float] = Field(
        ...
        description=(
            "Competition probability, which is determined as P_comp(x) = 1 / (C + 1)"
            " where C is the number of targets that overlap with target x"
        ),
        ge=0,
        le=1,
    )
    observability: Optional[float] = Field(
        ...
        description=(
            "Probability representing the likelihood of pointing to a target given the"
            " length of its visibility window and the time requested on target. The"
            " probability is represented by the ratio of the length of the observing"
            " window and the length of the visibility window"
        ),
        ge=0,
        le=1,
    )
    seeing: Optional[float] = Field(
        ...
        description=(
            "Seeing probability, whivh is derived from the cumulative distribution"
            " function of seeing measurements taken in Sutherland"
        ),
        ge=0,
        le=1,
    )
    total: Optional[float] = Field(
        ...
        description=(
            "Total probability, which is derived using the binomial theorem, where the"
            " number of trials are the number of tracks available to observe a target,"
            " the number of successes is the number of visits requested and the"
            " probability per trial is the total probability per track"
        ),
        ge=0,
        le=1,
    )
...
class PartnerName(str, Enum):
    ...
    COM = "Commissioning Proposals"
    DC = "Dartmouth College"
    DDT = "Director Discretionary Time Proposals"
    DUR = "Durham University"
    ENG = "Engineering Proposals"
    GW = "Gravitational Wave Event Proposals"
