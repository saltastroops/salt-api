...
from tests.conftest import (
    ...
    find_username,
    not_authenticated,
    authenticate_with_validation_token,
...
def test_user_verification_requires_authentication(
    ...
    response = client.post(f"{USERS_URL}/{user_id}/verify-user", json={})
...
def test_post_send_verification_link_returns_404_for_unknown_users(
    ...
    response = client.post(
        f"{USERS_URL}/send-verification-link",
        json={"username_email": "unknown@email.ac.za"},
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
    response = client.post(
        f"{USERS_URL}/send-verification-link", json={"username_email": "unknown"}
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_post_send_verification_link_send_email(
    ...
    mocker.patch.object(email_service_mock, "send_email")
    response = client.post(
        f"{USERS_URL}/send-verification-link", json={"username_email": user.username}
    )
    assert response.status_code == status.HTTP_200_OK
    assert "activation link has been sent" in response.json()["message"]

    response = client.post(
        f"{USERS_URL}/send-verification-link", json={"username_email": user.email}
    )
    assert response.status_code == status.HTTP_200_OK
    assert "activation link has been sent" in response.json()["message"]
